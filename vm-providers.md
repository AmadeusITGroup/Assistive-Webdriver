...
|  --- | --- |
|  [bufferToDataURI(buffer, mediaType)](./vm-providers.buffertodatauri.md) | Takes a buffer and returns the corresponding data URI with base64 encoding. |
|  [calibrationQRCodesGenerate(width, height, config)](./vm-providers.calibrationqrcodesgenerate.md) | Creates a calibration image containing QR codes. Each QR code encodes its position inside the image, allowing to easily detect the position of the whole image by reading any of its QR codes, which can be done with [calibrationQRCodesScan()](./vm-providers.calibrationqrcodesscan.md)<!-- -->. |
|  [calibrationQRCodesScan(image, config)](./vm-providers.calibrationqrcodesscan.md) | Scans an image containing a calibration image (which can be generated by [calibrationQRCodesGenerate()](./vm-providers.calibrationqrcodesgenerate.md)<!-- -->) and returns its position and the position of the QR code that was successfully scanned. |
...
|  [pngToBuffer(png)](./vm-providers.pngtobuffer.md) | Takes a png image and returns a buffer. |
|  [pngToDataURI(png)](./vm-providers.pngtodatauri.md) | Takes a png image and returns a data URI. |
...
|  --- | --- |
|  [CalibrationQRCodesConfig](./vm-providers.calibrationqrcodesconfig.md) | Configuration used when [generating](./vm-providers.calibrationqrcodesgenerate.md) or [scanning](./vm-providers.calibrationqrcodesgenerate.md) a calibration image. |
|  [CalibrationQRCodesScanResult](./vm-providers.calibrationqrcodesscanresult.md) | Calibration image scan result, as returned by [calibrationQRCodesScan()](./vm-providers.calibrationqrcodesscan.md)<!-- -->. |
